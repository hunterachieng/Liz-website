    ...
    <Layout>
      ...
      <div className="grid grid-cols-2 gap-4 px-20 py-10 sm:grid-cols-1 xsm:grid-cols-1 sm:px-5 xsm:px-5">
        <div>
          ...
          <p className="text-personaFaith py-5  ">
      ...
      <br />
      <h1 className="text-orange font-bold text-2xl px-20 sm:px-5 xsm:px-5">
        Findings
      </h1>
      <br />
      <div className="grid grid-cols-2 gap-4 px-20 py-10 sm:grid-cols-1 xsm:grid-cols-1 sm:px-5 xsm:px-5">
        ...
        <div className="w-11/12">
          ...
          <p className="text-text pt-5   ">
            Payments settlement was the biggest complaint from all the vendors
            interviewed
          </p>
          <br />
          ...
          <ul className="text-text   py-2 ml-5 w-5/6 leading-loose list-decimal md:w-full sm:w-full xsm:w-full">
            <li>
              There are delayed payments especially for consignment goods. Some
              vendors mentioned they have unsettled payments dating back to
              November 2018
            </li>
            <li>
              Vendors have to contact Masoko support to request for their
              account statements, a process that can lag and be time consuming
            </li>
            <li>
              Account statements are often unclear and contain errors. One of
              the statements we viewed was for the month of June but contained 2
              records from March
            </li>
          </ul>
        </div>
        <div className="w-11/12">
          ...
          <p className="text-text pt-5   ">
            In general, order processing did not appear to be a major stumbling
            block to the vendors using Masoko. However, the following complaints
            were raised by the majority of those interviewed:
          </p>
          <br />

          <ul className="text-text py-2 ml-5 w-5/6 leading-loose list-decimal md:w-full sm:w-full xsm:w-full">
            <li>
              New order notifications are received on email but become difficult
              to manage and become difficult to manage and process when they
              receive multiple orders on the same day. This led to some vendors
              going past the SLA agreement of fulfilling orders within 24 hours.
            </li>
            <li>
              At times, the emails do not contain the packing slip, so vendors
              have to contact support via email or phone call to request for the
              slip before shipping.
            </li>
          </ul>
        </div>
      </div>
      ...
      <img
        src={listing}
        alt="Masoko Product Listing"
        className="px-20 w-full sm:px-5 xsm:px-5"
      />
      <div className="px-20 py-10 sm:px-5 xsm:px-5">
        <h1 className="text-orange font-bold text-2xl">Recommendation</h1>
        <br />
        <h1 className="text-darkBlue  text-xl">The process</h1>

        <p className="text-taskHeading py-5 w-7/12 md:w-full sm:w-full xsm:w-full ">
          The research team facilitated a one day design workshop after the UX
          study in order to share lessons learned and agree on the features of
          the MVP.
        </p>
        <h2 className="text-personaFaith">Vendor journey</h2>
        <img
          src={journey}
          alt="Masoko Vendor Journey"
          className="px-20 w-full sm:px-5 xsm:px-5"
        />

        <p className="text-taskHeading   py-5 w-4/12 md:w-full sm:w-full xsm:w-full">
          The workshop participants included various stakeholders from the
          Masoko vendor management team as well as the design and development
          team from Masoko
        </p>
      </div>
      <div className="bg-grey">
        <div className=" px-20 py-10 sm:px-5 xsm:px-5">
          <h1 className="text-orange font-bold text-2xl">The Outcome</h1>
          <p className="text-taskHeading   py-5 w-10/12 md:w-full sm:w-full xsm:w-full">
            The idea was to validate as quickly as possible and Articulate the
            biggest problem with the current platform
          </p>
          <h1 className="text-personaFaith font-bold text-2xl">
            Onboarding Journey
          </h1>
          <p className="text-darkBlue text-xl   py-5">The process</p>
          <p className="w-10/12 pb-5  text-lightGrey md:w-full sm:w-full xsm:w-full">
            Most of the onboarding process required a human touch point with the
            Masoko team, either in person or on phone call:{" "}{" "}
            <span className="text-taskHeading">
              We simplified the process by automating all the processes by using
              Government registration tools for verification
            </span>
          </p>
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-1 xsm:grid-cols-1">
            ...
            <img
              src={productDetails}
              alt="Masoko Product Details"
              className="w-full"
            />
            <img
              src={singleProduct}
              alt="Masoko Single Product"
              className="w-full"
            />
          </div>
          <br />
          <h1 className="text-personaKamau text-2xl">Inventory Management</h1>
          <br />
          <p className="text-darkBlue text-xl py-5">
            Products Description Error Handling
          </p>
          <p className="text-lightGrey w-10/12 md:w-full sm:w-full xsm:w-full">
            Vendors have to contact Masoko support whenever they need to make
            any updates to their product, e.g for price, stock, description.:{" "}
            <span className="text-taskHeading">
              We introduced a{" "}{" "}
              <span className="text-personaFaith">sell as yours </span> feature
              that enabled vendors to sell similar products that meets Masoko
              quality standards, hence reducing conflict in description between
              Masoko content team and the vendors
            </span>
          </p>
          <br />
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-1 xsm:grid-cols-1">
            <img
              src={productListing}
              alt="Vendor Product Listing"
              className="w-full"
            />
            <img
              src={editSingle}
              alt="Vendor edit single product"
              className="w-full"
            />
            <img
              src={sellYours}
              alt="Vendor sell your product"
              className="w-full"
            />
            <img
              src={sellSingle}
              alt="Vendor sell single  product"
              className="w-full"
            />
          </div>
          <br />
          <h1 className="text-personaKamau text-2xl">Order Management</h1>
          <br />
          <p className="text-darkBlue text-xl py-5">Lack order progression</p>
          <p className="text-lightGrey w-10/12 md:w-full sm:w-full xsm:w-full">
            Vendors are unable to clearly see order progression on the platform
            from the time a new order is made to when it is delivered to the
            client:{" "}{" "}
            <span className="text-taskHeading">
              We introduced order status and new order tags to reduce the
              frustration vendors felt not being able to view order status and
              reason for return. In addition, each order received came with a
              packing slip solely for a particular order and not payment
            </span>
          </p>
          <br />
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-1 xsm:grid-cols-1">
            ...
            <img
              src={singleOrderItem}
              alt="Single Order Item"
              className="w-full"
            />
            <img src={orderSlip} alt="Single Order Item" className="w-full" />
          </div>
          <br />
          <h1 className="text-personaKamau text-2xl">Payment Management</h1>
          <br />
          <p className="text-darkBlue text-xl py-5">
            Difficulties in payments reconciliation
          </p>
          <p className="text-lightGrey w-10/12 md:w-full sm:w-full xsm:w-full">
            The structure of the account statements makes it difficult to
            reconcile payments. Vendors are unable to match a payment to a
            particular order: {" "}{" "}
            <span className="text-taskHeading">
              We introduced a payment grid on the platform that was simple and
              contained relevant payment information. additionaly we
              restructured the account statements to be an alignment with orders
              received in a particular duration of time.
            </span>
          </p>
          <br />
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-1 xsm:grid-cols-1">
      ...
      <div className="flex items-center justify-center">
        <button className="border bg-darkBlue py-2 px-4 text-white font-bold">
          <Link to="/case/prosper-case">View next casestudy</Link>
        </button>
        <button className="border border-darkBlue py-2 px-10 text-darkBlue font-bold ml-2">
          <a
            href="https://www.dropbox.com/s/64f51687txigid1/lizkagimbi%20Resume.pdf?dl=0"
            target="_blank"
          >
            Resume
          </a>
